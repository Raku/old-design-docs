...
A grammar is composed of methods introduces by one of the three keyword
...
involves a NFA, whiler the later involve the recursive engine. That's why the former is faster than the later.
...
short for sublanguage. A slang is a grammar derived from the Perl 6 grammar, and its associated L</action>s.
...
Slang is pinciipled way to create Domain Specific Languages (DSLs).
