...
Operation transforming a L</native> L</value> intro an objet.
...
Starting a self reliant system is an practical exercise on the chicken and egg problem.
...
In a L</dynamic language> like Perl 6, the compiler is also referred as L</interprete>r. 
In simpler dynamic languages like
Perl 5, the interpreter does not go thru conceptual phases similar to the one for a compiler
...
A composer is a syntax for defining L</values>. When values are L</object>s, their composer is an L</huffmanization>
of a L</constructor> L</expression>.
...
by L</Panda>. L</Rakudo *> include a tested subset of the ecosystem.
...
A way to use a git L<reposities|/repository> from one git repository.
...
An expression is constituted of operators and operands. More precisely it is made of an operator and
...
L<List|http://perlcabal.org/syn/S02.html#Grammatical_Categories> of categories.
...
involves a NFA, whiler the latter involve the recursive engine. That's why the former is faster than the latter.
...
A L</role> denoting an linearly ordered composite datastructure that can be iterated.
...
A role  can be composed with zero or more other roles, then instanciated into
...
Short for sublanguage. A slang is a L</grammar> derived from the Perl 6 grammar, and its associated L</action>s.
Alternatively the Perl 6 syntax
can be seen as the combination of many slangs (the regex slang, the quotation slang...)
Defining a slang has a cost because it usually involve generating new C</NFA>s table but except for
