...
A B<gen>erated B<sym>bol. Used primarily in macro parlance, a gensym acts as
...
symbols `#:G847`, `#:G848` and `#:G850` below from
...
the object in a human readable format, or a paste server used by denizens of
L</IRC>.
...
information can be regenerated from the bytecode.  In Rakudo the Single
...
needed (non-strict evaluation)".  In Perl6, strings and lists can be lazy
...
L</$_>, L</"$/"> and L<$!>. They are L</block> L</scope>d and are implicitly
defined at the beginning of each block.

=head2 MAIN

When present, a L</multi>L<sub> that is the entry point of a L</program> L</runtime>.
The code in L</phaser>s intented to run at compile time are executed before MAIN.
...
An opcode is an instruction in a bytecode.  Documentation about L</MoarVM>
...
L<Link|https://github.com/perl6/roast> to the L</github> L</repository> that
contains the said test suite. See also: L</roast>.
...
modules linking, be it in the L</OS> level, or at the Perl 6 L</VM> level
...
In L<parsing|/parser> theory, the parsed string is broken into tokens that
are the input of L<syntax analysis|/Syntax analysis>. In Perl 6, things are
not so clear cut and the two conceptual phases are defined by the
L</grammar>.
...
this internally. On Parrot and JVM, it sits on top of the native object
model.
