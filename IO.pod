...
Returns the next character from the input file attached to IO,
...
IO, if supplied, must be a filehandle object.  Indirect objects
...
=item IO.printf(FORMAT, LIST)

The function form works as in Perl 5.  The method form works on
filehandle objects, not formats.
