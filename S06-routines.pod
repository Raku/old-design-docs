...
copy parameters.  It may also be rebound to a raw parameter (see
"C<is raw>" below), but modification will fail as in the case where
an immutable value is bound to a C<raw> parameter.
