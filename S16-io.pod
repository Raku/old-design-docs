...
whether the specified path exists as a directory.  An optional C<:test>
parameter will apply the given file tests and fail if any of them is not
returning a true value.  The default for the file tests is C<< <r> >>.

  chdir($dir);   # sets $*CWD of scope, usually PROCESS::<$CWD>

  chdir($dir, :test<r> );  # default is only readable

The C<chdir()> function returns a C<X::IO::Chdir> Failure if the path does
not exist, or is not a directory, or fails the given file tests.  Otherwise
returns a newly created C<IO::Path> object (which will be C<True>).
...
system's temporary directory.  It can be set with the C<tmpdir()> function
which will check whether the specified path exists as a directory.  An
optional C<:test> parameter will apply the given file tests and fail if any
of them is not returning a true value.  The default for the file tests is
C<< <r w x> >>.

  tmpdir($dir);  # sets $*TMPDIR of scope, usually PROCESS::<$TMPDIR>

  tmpdir($dir, :test<r w x>); # default is read/write/executable

which will check whether the specified path exists as a directory and is
readable/writable/executable.  To set a locally scoped version of C<$*TMPDIR>,
you can use C<tmpdir()> with a temporary C<$*TMPDIR>:
...
It will return a newly created C<IO::Path> object (which is C<True>) or an
...
C<< <r w x> >>.
  ...
  homedir($dir, :test<r w x>); # default is read/write/executable 

which will check whether the specified path exists as a directory and is
readable/writable/executable.  To set a locally scoped version of C<$*HOME>,
you can use C<homedir()> with a temporary C<$*HOME>:
