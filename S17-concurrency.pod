...
The C<Promise.start> method also takes 2 optional named parameters:

    my $p = Promise.start( { ... }, :max(4), :id<image_processing> );

or, using the shorter C<start> sub:

    my $p = start { ... }, :max(4), :id<image_processing>;

=over 4

=item max

Indicate the maximum number of concurrently executing threads with this C<id>.
Only will start executing the given code when the number of concurrently
executing threads drops below the given maximum (for this C<id>).

=item id

An optional identifier to be used for checking on number of concurrent threads.
By default uses a string that is manufactured using the call-site information.
