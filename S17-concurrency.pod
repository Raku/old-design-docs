...
Produces a C<Supply> of Supplies.  Each C<more>'d C<Supply> corresponds to the
(single-value) mapping that was found for values C<more>'d on the given
C<Supply>.  Each C<more>'d value on the given C<Supply> is then C<more>'d on
the C<Supply> associated with the mapping.  Similar to C<List.classify>, but
does not support multi-level classification.
