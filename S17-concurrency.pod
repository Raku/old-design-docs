...
This allows a C<Supply>'s more/done/quit to be scheduled on another
...
Produces a C<Supply> in which the C<more>'d values are C<Pair>s consisting of
the classification value and the C<Supply> to which values of the given
C<Supply> will be C<more>d.  Similar to C<List.classify>, but does not support
...
given C<Supply> will be C<more>d.  Similar to C<List.categorize>.

=item reverse

  my $r = $s.reverse;

Produces a C<Supply> with the C<more>'s of the given Supply in reverse order.
...
Produces a C<Supply> with the C<more>'s of the given Supply in sorted order.
Please note that this C<Supply> will only start delivering values as soon as
...
state of the values to be C<more>d.
...
produces a C<Supply> which, when tapped, will C<more> any signal coming in.
For example:
...
But not much.  The workhorse of asynchronous IPC in Perl 6 is C<Proc::Async>:
    ...
    my $proc = Proc::Async.new( $path, *@args, :%ENV = %*ENV );
...
characters or bytes.
    ...
    $proc.stdout-chars.act(&say);   # simply pass it on to our $*OUT
    $proc.stderr-chars.act(&note);  # and $*ERR, but could be any code

or:

    my $stdout-bytes = $proc.stdout-bytes;
    my $stderr-bytes = $proc.stderr-bytes;
