...
The C<schedule> method takes a C<Callable> object and schedules it.
...
This returns, in order, the number of scheduled things that are not yet runnable due to
delays, the number of scheduled things that are runnable but not yet assigned to a thread,
and the number of scheduled things that are now assigned to a thread (and presumably running).
...
return anything but 0 for the currently running scheduled things, since they're waiting
...
The C<Publish> class (punned from the C<Supply> role) has various methodsi
...
C<Publish.interval> produces a C<Supply> that, when tapped, will produce an
...
different semantics, and so must provide a C<done>. This can be implemented
as follows:
