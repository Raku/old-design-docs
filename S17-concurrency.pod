...
A C<whenever> clause (described below) on a C<Channel> will fire for each received value, and also mark the whenever as "done" using the same criteria as C<.closed>, so it can also be used in order to write a reactor to receive from a channel until it is closed:
...
It is possible for values to be pumped into a C<Supply> from an asynchronous worker. In fact, it is possible for many threads to safely pump values into a supply. In the event this happens, the callback of a tap may be executed on many threads at the same time.

The C<Supply> class has various methods that produce more interesting kinds of C<Supply>.  These default to working asynchronously.  Most handle creation/retention/destruction of a C<Supplier> internally, or combine prexisting C<Supply> objects, so their users only need concern themselves with C<Supply> objects.
