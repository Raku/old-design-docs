...
The C<Supply> class (punned from the C<Supply> role) has various methods that produce more interesting kinds
of C<Supply>. These default to working asynchronously. Furthermore, they
start producing values upon the point of subscription.

C<Supply.for> takes a (potentially lazy) list of values, and returns a
C<Supply> that, when tapped, will iterate over the values and
invoke the C<more> callable for each of them, and any C<done> callable at the
end. If the iteration at some point produces an exception, then the C<fail>
callable will be invoked.

C<Supply.interval> produces a C<Supply> that, when tapped, will
