...
characters or bytes.
    ...
    $proc.stdout_chars.act(&say);   # simply pass it on to our $*OUT
    $proc.stderr_chars.act(&note);  # and $*ERR, but could be any code

or:

    my $stdout_bytes = $proc.stdout_bytes;
    my $stderr_bytes = $proc.stderr_bytes;
...
processing.  This can be achieved with the C<.close_stdin> method:

    $proc.close_stdin;

Finally, if your process as gong awry, you can stop it with the C<.kill>
method:
