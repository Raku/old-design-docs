...
The following methods are available on an instantiated C<Supply> (C<$s> in these examples).  Note that calling most of these methods on an on-demand C<Supply> does not constitute demanding a new set of values from the supply -- instead, tapping the resulting C<Supply> will do so, per C<Tap>.
...
Produces a C<Supply> in which the emit values are C<Pair>s consisting of the classification value and a C<Supply> to which values of the given C<Supply> will be emitted.  Similar to C<List.classify>, but does not support multi-level classification.
...
Produces a C<Supply> in which the emitted values are C<Pair>s consisting of zero or more classification values and a C<Supply> to which values of the given C<Supply> will be emitted.  Similar to C<List.categorize>.
...
These combinators that involve multiple supplies need care in their implementation, since values may arrive at any point on each, and possibly at the same time. To help write such combinators, the C<supply> block and C<whenever> clause are used.  These, along with the C<react> block, are available for general use as well.

A C<supply> block is a convenient way to create an on-demand supply.  It is just a declaration and does not run until the supply is tapped (which may happen multiple times resulting in multiple runs/clones of the block.)  Within a C<supply> block, emit can be used to emit values to the tapper, and done can be used to convey that there will be no more values.
...
The emit and done can be in nested scopes, and follow the same rules as C<gather> and C<take>, except they look for enclosing C<supply> or C<react> rather than a C<gather>.   An unhandled exception thrown inside of a supply block will be passed onwards using quit.

Likewise, a C<whenever> clause also looks around itself lexotically for an enclosing C<supply> or C<react>.  A C<whenever> clause takes a predicate which may be a C<Supply>, C<Channel>, C<Promise> or C<Iterable>, followed by a consequent block.  These clauses guarantee that only one thread will enter them or any other C<whenever> clause inside that same block at the same time -- blocking any C<.emit> call on a C<Supply> or leaving any additional values in a Channel's C<.send> queue in the scheduler.

A whenever with a C<Supply> as a predicate will tap that predicate when the enclosing block is run, providing its consequent block as the emit closure for the resulting C<Tap>.  However, it is sometimes best just to think of it as a loop which runs once for each value produced by the predicate C<Supply>.  Following this model, a C<QUIT> or C<LAST> phaser inside the consequent block may be used to provide a quit or done closure (respectively) to the C<Tap>.  The resulting C<Tap> itself is ensconced internally.

A whenever block with a C<Channel> as a predicate behaves analogously, running the closure once for every value sent by the scheduler.  A C<Promise> predicate runs the block when the C<Promise> is kept or broken.  An C<Iterable> predicate will start a new iterator when the whenever clause is run, entering the consequent closure for each value pulled from the C<Iterator>.

A C<supply> block containing C<whenever> clauses will call C<done> on itself when all C<whenever> clause predicates have had C<.done>/C<quit>, C<.close>/C<.fail> or C<.keep>/C<.break> called on them, appropriately, or in the case of a C<Iterable> predicate, when the end has been reached.  A C<react> block will not wait to be tapped -- it will immediately run and then wait for a done or quit condition, after which point, control will resume after the C<react> block.

Destructuring subsignatures may be used with whenever topics via pointy block syntax.
