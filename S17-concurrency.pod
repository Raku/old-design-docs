...
The first, known as the C<tap>, is invoked whenever a value is C<drip>ped into
...
drips will be done. The optional named parameter C<quit> specifies the code
...
methods C<drip>, C<done>, and C<quit>, which notify all current taps.
...
the C<drip> callable for each of them, and any C<done> callable at the end.
...
Produces a C<Supply> that will only C<drip> the B<last> given number (default
1) C<drip>s of the given C<Supply> when it is C<done>.

=item grab

  my $g = $s.grab( { .sort } ); # sort the values of a Supply

Produces a C<Supply> will grab all C<drip>s of the given C<Supply> until it
is done.  It will then call the given closure and then C<drip> each of the
...
C<drip>ped values) or a value to be smartmatched against.
...
run on another thread. It then C<drip>s a Supply (resulting in us having
a supply of supplies) that will either have a single value C<drip>ped and
...
This allows a C<Supply>'s drip/done/quit to be scheduled on another
...
Produces a C<Supply> that will C<drip> each reduction from the given C<Supply>,
...
Produces a C<Supply> that will C<drip> the characters coming in line by line
...
Produces a C<Supply> that will C<drip> the characters coming in word by word
from a C<Supply> that usually created by some asynchronous I/O operation.
...
Produces a C<Supply> in which the C<drip>ped values are C<Pair>s consisting of
the classification value and the C<Supply> to which values of the given
C<Supply> will be C<drip>ped.  Similar to C<List.classify>, but does not support
...
given C<Supply> will be C<drip>ped.  Similar to C<List.categorize>.

=item reverse

  my $r = $s.reverse;

Produces a C<Supply> with the C<drip>s of the given Supply in reverse order.
...
Produces a C<Supply> with the C<drip>s of the given Supply in sorted order.
Please note that this C<Supply> will only start delivering values as soon as
...
state of the values to be C<drip>ped.
...
hash with Pairs for C<drip>, C<done> and C<quit>.
...
specified along with the supply, is to use it for C<drip> and provide
...
keys for C<drip>, C<done> and/or C<quit>, instead of just a C<Callable>.
Given an array @s with Supplies:

  my $done = 0;
  my $result = on -> $res {
      @s => {
          drip => -> \val { $res.more(val) },
...
produces a C<Supply> which, when tapped, will C<drip> any signal coming in.
For example:
...
while the C<drip> closure is put in a C<do>. A C<Tap> is returned, which
