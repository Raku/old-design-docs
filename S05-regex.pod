...
get each individual sublist as a C<Capture> object. (That is, there is a C<@@()>
...
current regex on the action object, passing along the current C<Match> object as
the first positional argument, and the tag of the reduction (if any) as the second
argument.  The tag is supplied via a C<#=> comment on the same line as the C<{*}>.
