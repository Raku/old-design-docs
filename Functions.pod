...
QUESTION: should there be a:
 ...
 our Num multi prefix:<√> ( Num $x )

as another way to say sqrt?

ANSWER: Not in the standard language.  We're trying to avoid anything outside
of Latin-1 for standard imports.  As a special import it would be allowed,
much like the set theory operators, perhaps with a :UNI tag or some such.
...
To simply transform a list into a hash of arrays:
  ...
  say "The following people own red cars: ",
	%cars_by_color<red>.map:{.owner}.join ", ";
...
=item quotemeta
 ...
 our Str multi Str::quotemeta ( Str $string )
...
=item ord
 ...
 our Uni multi Str::chr( Uni $codepoint )
 our Uni multi Str::ord( Uni $character )
 multi Str::ord( Str $string )
...
numified and a single character when stringified. Thus, chr is just:
 ...
 our Uni multi Str::chr( Uni $codepoint) { $codepoint; }
