...
=item cat
 ...
 our Lazy multi Container::cat( Container *@containers );
...
=item each
 ...
 our Lazy multi Container::each( Bool :$shortest, Bool :$finite, Container *@containers )

Unlike Perl 5's each, C<each> is a general method that traverses any
composite container in a type-specific manner. C<each> is like C<zip>,
...
=item roundrobin
 ...
 our Lazy multi Container::roundrobin( Bool :$shortest, Bool :$finite, Container *@containers )

C<roundtobin> is very similar to C<each>, and in fact
...
=item zip
 ...
 our Iterator multi Container::zip ( Bool :$shortest, Bool :$finite, Array *@lists ) {
 our Iterator multi infix:Container::¥ ( Bool :$shortest, Bool :$finite, Array @a, Array @b ) {
 our Iterator multi infix:Container::Y ( Bool :$shortest, Bool :$finite, Array @a, Array @b) {
 our Iterator multi infix:Container::¥ ( Bool :$shortest, Bool :$finite, Iterator @a, Array @b) {
 our Iterator multi infix:Container::Y ( Bool :$shortest, Bool :$finite, Iterator @a, Array @b) {
...
This does what you might expect, but not in an obvious way. It
becomes:

 for infix:<¥>(infix:<¥>(@a;@b);@c) -> [$a,$b,$c] {...}

which zips C<@a> and C<@b>, and then passes the resulting iterator to
another C<¥> which interleaves C<@c>. This works because, when given an
C<Iterator> as its first argument, C<¥> will add each of the values
from its second paramter to the end of each tuple returned by the
iterator. This is just the mechanism, however. To the user,
the infix ¥ just does what they almost certainly expect it to do in the
first place.
...
=item crypt
 ...
 our Str multi method Str::crypt ( Str $plaintext: Str $salt )

Encrypts the string using a one-way hash function. This yields a string
which cannot be "decrypted". This is used for verifying strings such
as passwords. The C<$plaintext> is the text to be encrypted. The
C<$salt> is a string which controls how the encryption is done. Typically,
this is a random string of two characters that matches the following pattern:

 rule Str::CryptBasicSalt { <[./0-9A-Za-z]> ** 2 }

There are other formats of salt as well, though their use is not universal.
Check your operating system's B<crypt> function for more details. Often
...
characters of the string as significant).

If you have a previously encrypted string and a plaintext password, you
can check to see of the password is correct like so:

 sub checkpw ( $encrypted, $plaintext ) {
   return crypt($plaintext, $encrypted) ~~ $encrypted;
 }

This works because the first part of the encrypted form of the
password is the salt, so an encrypted text can be passed in place of
the salt (everything after the salt is ignored).
...
=item index
 ...
 our StrPos multi Str::index( Str $string, Str $substring, StrPos $pos = 0 )

C<index> searches for the first occurance of C<$substring> in C<$string>,
starting at C<$pos>.
...
=item pack
 ...
 our buf8 multi Str::pack( Str $template, *@items )
...
in terms of the final, internal represenation of the string, only
...
Templates are strings of the form:
  ...
  grammar Str::PackTemplate { 
   rule template  { [ <group> | <specifier> <count>? ]* }
   rule group     { \( <template> \) }
   rule specifier { <[aAZbBhHcCsSiIlLnNvVqQjJfdFDpPuUwxX\@]> \!? }
   rule count     { \* |
		    \[ [ \d+ | <specifier> ] \] |
		    \d+ }
 }
...
=item rindex 
 ...
 our StrPos multi Str::index( Str $string, Str $substring, StrPos $pos? )
...
=item sprintf
 ...
 our Str multi Str::sprintf ( Str $format, *@args )
...
similates its effect using C<%C>.
...
Optionally, you can use substr on the left hand side of an assigment
like so:
...
least-derived to most-derived order. See L<Synopsis 12: Objects>
...
=item ord
 ...
 our Str multi Str::chr( Int $codepoint, Str::CharSet $charset? )
 our Str multi Str::chr( Str::Codepoint $codepoint )
 our Str::Codepoint multi Str::ord( Str $character )

These functions are available for purposes of backward compatibility.
C<chr> takes a codepoint (an integer which can be indexed in the current
character set) and returns a C<Str> which contains only that codepoint.
If the entry matching the codepoint in the current character set
is combining or otherwise not intended for use as a stand-alone
grapheme, then C<chr>'s behavior will be implementation specific.

An alternate version of C<chr> takes a C<Str::Codepoint> object. See
below for further detail.

C<ord> behaves in a complementary fashion, taking the first codepoint
in C<$character> and returning it as a C<Str::Codepoint>. This object
may then be used as if it were an ordinary integer, but also retains
information about its originating character set. Thus,

 $s2 = chr(ord($s1));

is guaranteed to return the first character from C<$s1>, as long as that
"character" is represented by a single codepoint. Otherwise, its behavior
is implementation specific, and may result in an exception or undefined
value.
...
object, all of which can be found in the documetation for C<Time>.

There is, by default, no timezone associated with this Time object, so
whatever default the system has will take over if timezone-specifc
data is accessed.
...
information and the equvalent of the Perl 5 setpwent / endpwent functions.
...
signal at all, and is used to determine if processess are still running:
...
Important Conc::Proc::Status mehtods:
