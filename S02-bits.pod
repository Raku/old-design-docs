...
that happens to be the same as the keyword C<if>, you can still use it
as a non-keyword, as long as you don't put whitespace after it:

    my \if = 42; say (if) if if;   # prints 42

Here only the middle if of the second statement is taken as a keyword
because it has whitespace after it.  The other mentions of C<if> do not,
and would be illegal were it not that the symbol is defined in this scope.
If you omit the definition, you'd get a message like this:
...
You may use C<:!exists> to test for non-existence.  This is specifically handy
because of precedence rules making C<< !%hash<a> :exists >> apply the C<:exists> to the
...
anywhere in a name:
    ...
    OUTERS      # Symbols in the any outer lexical scope
...
Alternatively you can use a list of values, which is convenient for decimals:
...
C<:{...}> would be taken in one of two ways, depending on whether the
brackets define a closure or a hash.  If taken as a closure, C<:{...}>
creates a pair mapping the null key to the closure.  If taken as a hash
composer, the null key is ignored, and C<:{...}> creates an object-keyed
hash rather than the string-keyed hash that C<{...}> would without
the colon.)
...
to serve as the key.  A sigiled form that includes a twigil will not
include the twigil in the key.
