...
as the I<type object>.  The name itself returns that type object:
    ...
    Any         Perl 6 object (default routine parameter type, excludes Junction, Failure)
...
similar linguistic behaviors in English.)  The distrubutional behavior
...
C<Mu> or C<Failure>.  As a special case, a C<Failure> may be returned
from a function even if that function declares a return type constraint
that does not include the C<Failure> type.  (In fact, the attempt to
...
It is assumed that this downstream failure processing is robust enough
that the optimizer is allowed to assume that a function returns what
it declares as its return type, and most of the time omit the check for
failure on the return value.  Nearly all the failure checks are already
enforced downstream by the smartmatcher, usually through the binder.

Note that a C<Failure> object is undefined, but may contain one or
more defined C<Exception> objects, which are considered a normal
...
0 arguments here".  The C<Nil> value represents the absence of a value
...
an error -- preferably at compile time -- because it could be
