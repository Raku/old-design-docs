...
=head2 Thread methods and attributes

We intentionally do not list cross-machine parallelism Conc:: classes here.
Consult your local 6PAN mirror with a time machine.
 ...
 - attributes:
    - .started  # time
    - .finished # time
    - .waiting  # suspended (not diff from block on wakeup signal)
