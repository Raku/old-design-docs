...
It is an error for a "level-R<N+1>" C<=item> block (e.g. an C<=item2>,
C<=item3>, etc.) to appear anywhere except where there is a preceding
"level-R<N>" C<=item>. That is, an C<=item3> can only be specified if an
C<=item2> appears somewhere before it, and that C<=item2> can only
appear if there is a preceding C<=item1>.
