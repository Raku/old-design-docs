...
retained. Finally, all of this needs to be done in a way that provides much
of the convenience that a Perl is expected to provide, while still having a
...
an C<Array> but does not wrap it into a C<Scalar> container. The forth will do
two iterations, since the argument is a list of two things; that they both
...
The only one of these that is likely to provide a surprise is C<[[1]]>, but is
...
return a C<Seq>, as do most synchronous sources of multiple values.
    ...
    say (1, 2, 3).map(* + 1).WHAT;  # Seq
    say (1, 2 Z 'a', 'b').WHAT;     # Seq
    say (1, 1, * + * ... *).WHAT;   # Seq
    say $*IN.lines.WHAT;            # Seq
...
C<cache> will also fail. It is only C<.cache> that may be called once.
