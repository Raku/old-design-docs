...
Some of Perl 6 Pod's uppercase block typenames are used to indicate
...
short).  For example, if a compunit contains this pod:

    =NAME     Dog - our favorite pet
    =AUTHOR   cpan:JRANDOM - Joe Random
    =VERSION  1.2.1

When you attempt to load a compilation unit, like:
...
To declare that a compunit emulates an older version, declare it the uppercase
block typename EMULATES.  For instance:
...
installed.  The optional second parameter specifies the slang in which the
compunit is written.  It defaults to "perl6".  Returns whether compilation
unit was installed.
