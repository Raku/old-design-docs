...
Synopsis 11: Units, Modules and Packaging
...
Some of Perl 6 Pod's uppercase block typenames may be used to indicate
meta-information that is needed to add a compunit to a given Perl 6
installation (for instance with L<CompUnitRepo::Local::Installation>, as well
as being able to load the compunit at a later time.  These are:

=over 4

=item =NAME

This is the (long) name of the compunit, which with the compunit can be found
with C<use>, C<need> or C<require>.  To keep this compatible with older uses
of NAME, only the first \S+ of the string will actually be used. So:

  =NAME Foo::Bar - making Bar out of Foo

would only use "Foo::Bar" for the (long) name of the compunit.

=item =AUTHOR

This is the I<auth> of the compunit.  It is supposed to be a URI identifying the
author or authorizing authority. Only the first \S+ of the string will actually
be used.  For example:

 =AUTHOR cpan:JRANDOM - Joe Random

would only use I<cpan:JRANDOM> as the I<auth>.

=item =VERSION

This is the I<ver> of the compunit.  Also, only the first \S+ of the string will
actually be used.  For example:

 =VERSION 2013.06 - Poznan

would only use I<2013.06> as the I<ver>.

=back

The combination of (long) NAME, AUTHOR (auth) and VERSION (ver), we call the
I<credentials> of a compunit.
...
To declare that a compunit emulates an older version, you can declare it
with the uppercase block typename EMULATES.  For instance:

 =NAME     Dog - man's best friend
 =AUTHOR   cpan:JRANDOM - Joe Random
 =VERSION  1.2.1
 =EMULATES 1.2.0

would indicate this compunit emulates version 1.2.0 itself.

Or to simply exclude use of the older module and (presumably) force
the user to upgrade, you can say:

 =EXCLUDES 1.2.0

The string is parsed like a C<use> wildcard, and you can have more than one,
...
Please note that all of these are only potential default values for installing
a compunit with the L<CompUnitRepo>'s C<install> interface.  Any CompUnitRepo
implementation may choose to ignore these hints in favor of other explicit or
...
CPAN mirror or github repository.  Returns the instantiated object.
...
=head2 install
 ...
 my $installed = $repo.install(
   ...
   :$longname,    # e.g. Dog
   :$auth,        # e.g. cpan:JRANDOM
   :$ver,         # e.g. 1.0.3
   :$emulates,    # e.g. 1.0.2
   :$excludes,    # e.g. 1.2.0
   :$created,     # e.g. 2013.12.04
   :$from         # most likely "perl6"
 );
...
The object that describes a compilation unit.  Is expected to have the
following attributes/methods: longname, auth, ver, emulates, excludes, created
and from, corresponding to the parameters as specified in L<CompUnitRepo>'s
C<install> interface.
