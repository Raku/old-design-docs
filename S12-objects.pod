...
If the declared enum typename (the outer name) begins with an uppercase letter, the enum values
...
actual type of the enum object returned by using the symbol is the enum type itself.
...
The return value of an anonymous enum is an c<EnumMap>.
