...
=head1 PROPOSED FEATURES

=head2 Sigils imply :D
...
=head2 Formal Rules for Defining Matched Delimiters/Brackets
...
=head2 Remove deprecated Test::is_approx
...
=head2 Properly reserve all C<< :sym<> >> colonpairs on subroutines
...
=head2 Use IEEE 754-2008 semantics for num/Num infix:</>, infix:<%>, and infix:<%%>
...
=head2 Remove dummy precision parameters from Rational/Int .Rat and .FatRat coercers
...
=head2 Deprecate IO::Path.chdir and IO::Handle.slurp-rest

Per IO Grant work.

=over 4

=item * C<IO::Path.chdir> - the name is a poor choice, as it doesn't B<ch>hange
    any B<dir>ectories. Functionality with relative-path argument is directly
    replaced by C<IO::Path.add> method and functionality with absolute-path is
    directly replaced by C<IO::Path.new> or C<Cool.IO> methods.

=item * C<IO::Handle.slurp-rest> - removed to make interface consistent with
    the rest of the language that uses C<slurp> as the name for the routine.
    The C<IO::Handle.slurp> was already implemented as part of the IO Grant.
...
=head2 Swap IO::Path.child to use .child-secure's code
...
=head2 Make all redeclarations fatal
...
=head2 Remove Str.lines :$count parameter
...
=head2 Remove $/ magicalness from %() and @()
...
=head2 Remove $*MAIN-ALLOW-NAMED-ANYWHERE
...
=head2 Make $*ARGFILES := $*IN or IO::ArgFiles.new($*IN) inside MAIN
...
=item * https://rt.perl.org/Ticket/Display.html?id=131703#txn-1471796

=item * https://irclog.perlgeek.de/perl6/2017-07-05#i_14827209

=item * https://irclog.perlgeek.de/perl6-dev/2017-07-05#i_14827981
...
=head2 De-magicalize '-' in IO::Handle.open

Currently, C<'-'.IO> will operate on a filesystem object (e.g. C<'-'.IO.mkdir>
will create a directory named C<->). However, trying to C<.open> such a path
causes special-cased behaviour in C<IO::Handle.open> where it return $*OUT
...
The proposal is to remove this feature in 6.d, so that C<'-'.IO.spurt: 'foo'> would
spurt into a file named C<-> rather than to $*IN handle.
