...
=item titlecase
 ...
 our Str multi method titlecase ( Str $string: :&filter = &lc, :%exceptions = set() ) is export
...
=item comb
 ...
 our List multi comb ( Str $matcher, Str $input, Int $limit = Inf, Bool :$match )
 our List multi comb ( Regex $matcher, Str $input, Int $limit = Inf, Bool :$match )
 our List multi method comb ( Str $input: Str $matcher, Int $limit = Inf, Bool :$match )
 our List multi method comb ( Str $input: Regex $matcher = /./, Int $limit = Inf, Bool :$match )
...
format token. Format tokens have the following grammar:
 ...
 grammar Str::SprintfFormat {
  ...
  token directive { < % c s d u o x e f g X E G b C p n i D U O F > }
 }
