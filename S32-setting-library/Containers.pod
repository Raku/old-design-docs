...
=item first-index
 ...
 multi method first-index ( @values: Matcher $test )
 multi first-index ( Matcher $test, *@values )
...
=item pick
 ...
 multi method pick ( @values: Int $num --> Seq )
 multi method pick ( @values: Whatever --> Seq )
 multi pick ( Int $num, *@values --> Seq )
 multi pick ( Whatever, *@values --> Seq )
...
of an iterabl container of one element.

=item roll

 multi method roll ( --> Mu )
 multi method roll ( @values: Int $num --> Seq )
 multi method roll ( @values: Whatever --> Seq )
 multi roll ( Int $num, *@values --> Seq )
 multi roll ( Whatever, *@values --> Seq )
...
the invocant list instead of a one-elemented list or sequence.
...
=item map
 ...
 multi method map ( @values: Code *&expression --> Seq )
 multi map ( Code $expression, *@values --> Seq )

C<map> returns a lazily evaluated sequence which is comprised of
...
=item duckmap
 ...
 multi method duckmap ( @values: Code *&expression --> Seq )
 multi duckmap ( Code $expression, *@values --> Seq )
...
=item deepmap
 ...
 multi method deepmap ( @values: Code *&expression --> Seq )
 multi deepmap ( Code $expression, *@values --> Seq )
...
will be an array containing every value in the list.  The return value
