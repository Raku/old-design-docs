...
to use multi-method dispatch on both subroutines as well as methods to
increase the performance or maintainability.
...
X<chdir()>
  ...
  sub chdir(Str() $dir, Str() $CWD = $*CWD --> IO::Dir)
...
positional parameter, should be taken.  Returns the C<IO::Dir> object of the
directory if successful (which is C<True>), or an appropriate C<Failure>.
...
X<indir()>
  ...
  sub indir(Str() $dir, &what, Str() $CWD = $*CWD)
...
X<mkdir()>
  ...
  multi sub mkdir(Str() $dir, Int $mode = 0o777 --> Bool)

Create directory indicated by the first positional, with the (optional) mode
as the second positional.  Return C<True> if successful, or an appropriate
C<Failure>.
...
X<move()>
  ...
  sub move( Str() $src, Str() $dest, Bool :$createonly --> Bool )
...
that directory.  If :createonly is set to C<True>, the move fails if a
...
Encoding to use if opened in text mode.  Defaults to "Unicode", which implies
figuring out which actual UTF is in use, either from a BOM or other heuristics.
...
an object doing the C<IO::Pathy> role, or an object doing the C<PIO> role).
...
positional parameter was B<not> an object doing the C<PIO> role:

=over 4

=item :enc

The encoding to pass on to L</open()>.
...
X<copy()>
    ...
    sub copy(Str() $src, Str() $dst, :$createonly --> Bool)
...
X<rename()>
    ...
    sub rename( Str() $src, Str() $dst, :$createonly --> Bool )

Moves a file, as indicate by the first positional parameter, by renaming it
...
X<tmpdir()>
  ...
  sub tmpdir(Str() $dir, Str() $CWD = $*CWD --> Bool)
...
This set of modules was inspired by Perl5's C<File::Spec>.  An implementation
...
X<.spurt>
    ...
    method spurt(:$enc, :$append, :$createonly  --> Str|Buf)
