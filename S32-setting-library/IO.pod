...
Returns a lazy list of (relative) path names in the C<$directory> as strings,
...
Copies a file, as indicate by the first positional paramater, to the
...
Moves a file, as indicate by the first positional paramater, by renaming it
...
Moves a file, as indicate by the first positional paramater, by copying its
...
X<unlink()>
    ...
    sub unlink($filename as IO --> Bool) is export

Delete an ordinary file, link, or symbolic link.  Returns True on success,
or an appropriate C<Failure>.
