...
method responds to a coercion request, the multi C<COERCE> will
be called with the argument (in this case, C<$spot>) as the first
parameter, and the destination type (in this case, C<Dog>) as the
second.  The default implementation for C<COERCE(Any, Any)> is
to call a method on the source object with the same name as the
destination class, as seen below.
...
Of course, you can always provide a candidate for C<COERCE>, which is especially
useful if you want to provide coercions between classes that don't know about
each other, you have two potential destination classes that share the same name,
or you're coercing to/from a class with no name at all.
